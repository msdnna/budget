...
// event. Error payloads are tiny gin.H{"error": ...} JSON, so a few KiB is
// plenty and bounds the per-request memory cost.
const maxCapturedBody = 4 << 10 // 4 KiB

// capturingWriter tees a bounded prefix of the response body so SentryReport can
// attach the actual error message (e.g. the underlying Mongo error) to the event.
...
// failures by writing http.StatusInternalServerError directly (a MongoDB outage,
// for instance, surfaces as a 500 on every DB-backed route), so capturing by
// status code reports those without touching each handler. sentrygin already
// covers panics; this fills the gap for handled errors.
//
...
func SentryReport() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		hub.WithScope(func(scope *sentry.Scope) {
			...
			if len(details) > 0 {
				scope.SetContext("budget", details)
			}
			// Group per route+status rather than per concrete URL (IDs vary),
			// so all 500s on the same endpoint collapse into one issue.
