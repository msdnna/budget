<p align="center">
  <img src="frontend/public/favicon.svg" alt="mb — Семейный бюджет" width="220" />
</p>

<h1 align="center">Семейный бюджет</h1>

| Компонент | Lint + Test | Build | Coverage | Release |
|-----------|-------------|-------|----------|---------|
| Backend (Go)     | [![lint+test](https://img.shields.io/github/actions/workflow/status/msdnna/budget/ci.yml?branch=main&jobName=Backend&label=)](https://github.com/msdnna/budget/actions/workflows/ci.yml) | [![build](https://img.shields.io/github/actions/workflow/status/msdnna/budget/ci.yml?branch=main&jobName=Backend%20build&label=)](https://github.com/msdnna/budget/actions/workflows/ci.yml) | [![coverage](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fmsdnna%2Fbudget%2Fbadges%2Fbackend.json)](https://github.com/msdnna/budget/actions/workflows/ci.yml) | [![release](https://img.shields.io/github/actions/workflow/status/msdnna/budget/release-api.yml?label=)](https://github.com/msdnna/budget/actions/workflows/release-api.yml) |
| Web (Vue 3)      | [![lint+test](https://img.shields.io/github/actions/workflow/status/msdnna/budget/ci.yml?branch=main&jobName=Web&label=)](https://github.com/msdnna/budget/actions/workflows/ci.yml) | [![build](https://img.shields.io/github/actions/workflow/status/msdnna/budget/ci.yml?branch=main&jobName=Web%20build&label=)](https://github.com/msdnna/budget/actions/workflows/ci.yml) | [![coverage](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fmsdnna%2Fbudget%2Fbadges%2Fweb.json)](https://github.com/msdnna/budget/actions/workflows/ci.yml) | [![release](https://img.shields.io/github/actions/workflow/status/msdnna/budget/release-web.yml?label=)](https://github.com/msdnna/budget/actions/workflows/release-web.yml) |
| Android (Kotlin) | [![lint+test](https://img.shields.io/github/actions/workflow/status/msdnna/budget/ci.yml?branch=main&jobName=Android&label=)](https://github.com/msdnna/budget/actions/workflows/ci.yml) | [![build](https://img.shields.io/github/actions/workflow/status/msdnna/budget/ci.yml?branch=main&jobName=Android%20build&label=)](https://github.com/msdnna/budget/actions/workflows/ci.yml) | [![coverage](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fmsdnna%2Fbudget%2Fbadges%2Fandroid.json)](https://github.com/msdnna/budget/actions/workflows/ci.yml) | [![release](https://img.shields.io/github/actions/workflow/status/msdnna/budget/release-android.yml?label=)](https://github.com/msdnna/budget/actions/workflows/release-android.yml) |
...
### Финансовый учёт
- **Доходы и расходы** с произвольными per-section категориями (свои наборы для расходов, доходов и листа желаний), источниками/целями, описанием
- **Начальный баланс** — отдельный тип транзакции, учитывающийся в балансе, но исключённый из статистики доходов; задаётся per-deposit (банк / наличные)
- **Deposit scope** — разделение бюджета на банковский счёт и наличные: глобальный сегмент в шапке, фильтр на всех агрегациях, per-deposit начальный баланс
- **Список желаний** с собственными категориями и привязкой исполненной покупки к существующей расходной транзакции
- **Detail requests** (запросы на детализацию) — split одной крупной транзакции на несколько дочерних с сохранением суммы и исключением parent'а из статистики
- **Регулярные платежи** — единая модель с listом желаний (frequency: monthly / weekly / yearly / once), «Оплачено» / «Куплено» автоматически создают связанную транзакцию
- **Лимиты расходов на категорию** с месячным сбросом и push-уведомлениями всему family-кругу при приближении/превышении
- **Множественный выбор** на любом списке (long-press на Android, чекбоксы на вебе) для массового удаления
- **Inline-редактирование** записей по полям (карандаш → инпут) без модальных окон; tap-to-edit на мобильном вебе
...
- **Donut-диаграмма** в цветах категорий с иконками; tap по сегменту → переход на отфильтрованный список (web + Android); группировка мелких категорий в «Прочее» с раскрытием
- **Период** статистики — двусторонняя синхронизация: tile-picker дат ⇄ перетаскивание по графику (ECharts brush)
- **Прогноз** трат и доходов на ближайшие месяцы с раскладкой по регулярным платежам и листу желаний
- **Экспорт** в Excel и PDF (PDF — с DejaVu Sans, поддерживает кириллицу)
- **Snapshot import/export** — JSON-снимок (пользователи + категории + иконки base64 + транзакции + лист желаний + DRs) для переноса между инстансами; merge-skip / replace-keep-caller; first-run wizard при пустой базе

### Аутентификация и пользователи
- JWT с **rotating refresh tokens** (access HS256 TTL 24 ч, отдельный refresh с `jti`); регистрация через UI не предусмотрена
- Создание пользователей через CLI: `make create_user USER_LOGIN=... USER_PASSWORD=... USER_NAME=...`; первый пользователь автоматически становится `is_admin`
- **Admin-панель управления пользователями** (`/settings/users`): soft-delete, блокировка, смена пароля, аватары — split-pane на десктопе, отдельная вкладка на мобильном
- В каждой записи проставляется `created_by` / `last_modified_by` (id, имя, аватар); при правке профиля snapshot'ы догоняются во всех коллекциях автоматически
- **App lock** на Android — PIN (PBKDF2) + биометрия, тайм-аут, восстановление паролем

### Android-приложение
- **Native UI** на Jetpack Compose, Material 3, светлая/тёмная тема
- **Офлайн-режим** с двусторонней синхронизацией: Room + WorkManager + `/api/sync/pull|push` с разрешением конфликтов через UI; global sync-banner + foreground service для больших pull'ов
- **In-app обновления** — баннер + обязательный диалог при критическом обновлении; APK загружается с сервера (`/apks/<file>.apk`)
- **Унифицированный chip-фильтр** на Income / Expenses / Statistics / Forecast — общий `FilterCard` shell с группами «Период» / «Категории» / «Счёт»; long-press на funnel сбрасывает фильтр
- **Reachability gate** — TCP-probe + OkHttp-interceptor для мгновенного offline-индикатора при недоступном сервере
- **Server discovery** — сканирование подсети по `/api/health` для автоопределения сервера
- **Уведомления-напоминания** + push'и по лимитам категорий — daily / weekly / monthly / quarterly с picker дней и времени
- **Pull-to-refresh** + skeleton-загрузка во всех списках

### Веб-интерфейс
- Vue 3 + Naive UI, светлая/тёмная тема (свой `tokens.js` + CSS-переменные), русская локаль для пикеров (`ruRU` + `dateRuRU`)
- Tile-grid period picker (4×3 month/year), синхронизирован с brush-выделением на графике
- Per-view stores транзакций (фильтры/пагинация изолированы между Income и Expenses)
- **Адаптивные таблицы** на десктопе — actions-popover ниже 740px, fixed-slot колонки, split-pane на settings
- **Мобильный UX** — FAB-«+» вместо top-button, `BulkFabRow` 2-tap confirm на bulk-удаление, swipe-actions на карточках, fade/slide-анимации
- **Input history autocomplete** на полях «Источник» / «Назначение» / «Название» — localStorage, кап 20
