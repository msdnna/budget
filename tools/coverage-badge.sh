...
# under _badges/<component>.json. The badges branch (orphan) hosts these
# files; README references them via
...
#   backend  → Go coverprofile  (e.g. backend/coverage.out)
#   web      → LCov info file   (e.g. frontend/coverage/lcov.info)
#   android  → JaCoCo XML report
set -euo pipefail

COMPONENT="${1:?component name required (backend|web|android)}"
...
PY
    ...
    ;;
  *)
    echo "Unknown component: $COMPONENT (expected backend|web|android)" >&2
    exit 1
    ;;
esac
