        ...
        <div class="dr-progress" :style="progressCardStyle">
          <div class="dr-progress-row">
            ...
            <span class="dr-progress-num">
              <span :class="{ blurred: valuesHidden }">{{ totalChildren.toLocaleString('ru-RU') }}</span>
              /
              <span :class="{ blurred: valuesHidden }">{{ targetAmount.toLocaleString('ru-RU') }}</span>
              ₽
            </span>
          </div>
          <n-progress
          ...
          <div class="dr-progress-meta">
            ...
            <span v-if="overshoot > 0" class="dr-overshoot">+{{ overshoot.toLocaleString('ru-RU') }} ₽ сверх</span>
            <span v-else-if="remainder > 0" class="dr-remainder">остаток в баланс: {{ remainder.toLocaleString('ru-RU') }} ₽</span>
          </div>
        </div>
    ...
    <template #footer>
      <n-space justify="space-between" align="center">
        <n-text depth="3" style="font-size: 12px">
          <span v-if="view?.request?.assignee">Исполнитель: {{ view.request.assignee.display_name }}</span>
          <span v-if="view?.request?.creator"> · Создал: {{ view.request.creator.display_name }}</span>
        </n-text>
        <n-space>
...
import {
  NModal, NSpin, NCard, NList, NListItem, NSpace, NText, NButton,
  NPopconfirm, NEmpty, NProgress, NForm, NFormItem, NGrid, NGridItem,
  NInputNumber, NDatePicker, NSelect, NInput, useMessage,
} from 'naive-ui'
...
const childForm = ref({ amount: null, date: Date.now(), category: '', purpose: '', description: '' })
...
watch(() => [props.show, props.requestId], ([s, id]) => {
  if (s && id) load()
...
<style scoped>
.dr-progress-row { display: flex; justify-content: space-between; align-items: baseline; }
.dr-progress-label { font-size: 12px; opacity: 0.7; text-transform: uppercase; letter-spacing: 0.05em; }
.dr-progress-num { font-size: 16px; font-weight: 700; font-variant-numeric: tabular-nums; }
.dr-progress-meta { display: flex; justify-content: space-between; margin-top: 6px; font-size: 12px; opacity: 0.65; }
