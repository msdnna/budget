...
#### Added
- **Intent-триггеры для telegram-бота** — новая коллекция `intent_triggers` (one-doc-per-intent: `{intent, phrases[]}`) + 2 эндпоинта: `GET /api/intent-triggers` (любой авторизованный, возвращает все 4 известных намерения — `wishlist`, `recurring_payment`, `link_existing`, `detail_request` — с пустым списком если фраз нет) и `PUT /api/intent-triggers/:intent` (admin, полная замена списка фраз). Фразы триммятся + дедупятся case-insensitive в repo. Намерение `transaction` — дефолт, триггеров не несёт.
- **Встроенные триггеры засеваются в БД как обычные записи.** `models.DefaultIntentPhrases` — единственный источник правды; `IntentTriggerRepository.EnsureDefaults` (вызывается на старте рядом с `catRepo.EnsureDefaults`) сидит фразы по намерению **только если документа ещё нет** → правки/удаления админа не откатываются при рестарте. Бот собственного хардкода фраз больше не держит.
- **`/api/telegram/context` теперь отдаёт `intent_triggers`** — `map[intent][]phrase` (только намерения с ≥1 фразой). БД — источник правды; бот использует их как есть (без merge с хардкодом).

#### Changed
- `NewTelegramHandler` принимает дополнительный `*repository.IntentTriggerRepository` (nil-safe в `Context`). Один caller — main.go.
...
#### Changed
- **Подсветка перехода/фокуса — в цвет темы (primary) вместо оранжевого.** В родительско-дочерних записях (Доходы/Расходы, десктоп + мобила) primary только на бордере — фон группы сохраняется. В Прогнозе (у итемов нет группового цвета) — мягкая primary-заливка + primary-бордер. Цвет проброшен в scoped-CSS через `v-bind(primaryColor)` (+ `focusBg` = `primaryColor+1f` для Прогноза), реактивно следует за сменой темы.

### [1.45.3] — 2026-06-01

#### Fixed
- **Подсветка перехода рисовала бордеры по каждой ячейке** (внутренние колоночные границы) — на строках Доходов/Расходов И в Прогнозе. Заменено на единую обводку строки: top/bottom inset на всех ячейках + left только на первой и right только на последней — один внешний 1px-бордер без вертикалей внутри.

#### Added
- **Навигация родитель↔дочерний в группах — на мобильном фронте.** Порт десктоп-фичи на карточки Доходов/Расходов: тег «↑ К родителю» на дочерних (split/ЗнД) + «Разделено»/«Детализировано» на родителе; тап по «К родителю» включает show-toggle, подгружает и подсвечивает родительскую карточку (внешнее 1px-кольцо `.tx-card-focus`, scroll-into-view). `@click.stop` гасит открытие edit-формы карточки.

### [1.45.2] — 2026-06-01

#### Fixed
- **Группы (split/DR) «мигали» при наведении** — Naive подменял пастельный тинт строки на свой серый hover-фон рывком. Тинт каждой группы вынесен в RGB-переменную `--grp` на строке; hover теперь усиливает тот же тинт (`rgba(var(--grp),.24)`) с `transition`, без скачка цвета.
- **Подсветка по переходу (Прогноз) — тонкая и стойкая.** Обводка 2px→1px, фон легче; убран авто-сброс через 2.5с (`focusedId` держится) — найденный итем не теряется из виду.
- **Мобильная подсветка по переходу не показывалась** (inset-обводка пряталась за непрозрачной карточкой) — заменена на внешнее кольцо `box-shadow 0 0 0 1px` вокруг `.fc-card-focus`.
- **Мобильный «двойной клик» при переходе по привязке** — после `router.push` на Прогноз synthetic ghost-click попадал на сфокусированную карточку и открывал её редактирование. `onForecastTap` игнорирует тапы 700мс после фокуса (`tapSuppressUntil`).

#### Added
- **Навигация родитель↔дочерний в группах (десктоп).** В Доходах (split) и Расходах (ЗнД) у дочерних записей — кликабельный тег «↑ К родителю»: включает фильтр «показать разделённые/детализированные» (родитель скрыт по умолчанию), подгружает и подсвечивает родительскую строку (стойкая 1px-обводка). Родитель помечен статичным тегом «Разделено» / «Детализировано» — теперь видно, кто есть кто.

### [1.45.1] — 2026-06-01

#### Fixed
- **Полоса вкладок настроек переполнялась и не скроллилась**, а подписи переносились на две строки. `.settings-tabs` (и `.forecast-tabs` в Прогнозе) теперь скроллятся горизонтально (`overflow-x:auto` + `flex-wrap:nowrap`, скроллбар скрыт), вкладки `flex:1 0 auto` + `white-space:nowrap` — на десктопе заполняют ширину, на мобиле уходят в скролл без переноса.
- **Кнопка удаления в глоссарии слишком мелкая** для тача — заменена с `size=tiny` «×» на `size=small` с иконкой корзины (≈ размер actions-кнопки в доходах/расходах), на десктопе тоже крупнее.

### [1.45.0] — 2026-05-29

#### Added
- **`/settings/intent-triggers` — admin-страница «Триггеры бота».** Карточка с 4 секциями намерений (Желаемые покупки / Регулярные платежи / Привязать расход / ЗнД), в каждой `n-dynamic-tags`. Сохранение per-intent через `PUT /intent-triggers/:intent` (требует api 1.29.0). Встроенные фразы засеяны в БД и редактируются/удаляются как обычные записи.
- **Sidebar/mobile-nav + SettingsTabs.** Новая под-вкладка «Триггеры бота» (admin-only) между «Глоссарий» и «Импорт/экспорт».
- **API client** — экспорт `intentTriggers` с `list/update`.
...
#### Added
- **Меню команд + навигация.** `commands.py` публикует `set_my_commands` (RU-описания) + menu-button при старте; `/help` показывает все возможности и команды. Команды: `/start /help /link /dr /dr_close /cancel /unlink`.
- **Intent-роутер.** Двухступенчатый pipeline: `intents.classify_intent` (strict-JSON классификатор) определяет намерение (`transaction|wishlist|recurring_payment|link_existing|detail_request`), затем диспатч в специализированный экстрактор. Триггер-фразы целиком из БД (`/api/telegram/context`) — в боте никакого хардкода (встроенный набор сидится бэкендом). `transaction` — безопасный fallback.
- **Deposit-детекция.** В парсер транзакции добавлено поле `deposit` (`bank|cash`): «наличными/налом/кэшем» → cash, иначе bank. Проброшено через схему, few-shot, draft, confirm-экран и `POST /transactions`.
- **🛒 Желаемые покупки.** «хочу/планирую купить … за …» → `parse_wishlist` → confirm → `POST /wishlist` (frequency=once). Поля: name (без «хочу купить»), estimated_cost, notes (магазин/деталь), category из wishlist-секции.
- **🔁 Регулярные платежи.** «оплатил счёт за воду …» → `parse_recurring` матчит сообщение к `regular_items` (из `/statistics/forecast`), создаёт expense с `wishlist_id` + унаследованными category/deposit/purpose, числовые данные — из запроса. Нет совпадения → fallback на обычный расход.
- **📋 Доход по шаблону.** Для income: если в истории по категории >2 однотипных записей — клонируется раскладка полей (source/description/deposit), меняются только сумма + дата. Вариант «Аванс» матчится отдельно (с/без маркера). Иначе — обычный LLM-разбор.
- **🔗 Привязка существующего расхода.** «привяжи расход X от DD.MM к регулярному/желаемому Y» → `parse_link` → поиск unlinked-расхода (`?unlinked=true`) + цели (по имени/категории/типу) → confirm → `POST /wishlist/:id/link/:tx_id`.
- **📝 ЗнД (запросы на детализацию).** Создание: «создай ЗнД на <кого> на <сумму> категория <X>» → резолв исполнителя по `/api/users` → confirm → создание lump-sum расхода + `POST /detail-requests`. Заполнение: `/dr` (список открытых на меня → выбор) → режим накидывания child-расходов (текст/голос, каждое = `POST /detail-requests/:id/transactions`) → `/dr_close` закрывает.

#### Changed
- `_transcribe_voice` вынесен из `fallback_voice` — переиспользуется в ЗнД-fill голосовом флоу.
- `api_client` расширен методами: wishlist-категории, create_wishlist, regular_items, list_transactions (income-шаблон / unlinked), list_wishlist, link_expense, list_family, detail-request CRUD.

#### Tests
- `tests/test_intents.py` — форма `ALL_INTENTS` + рендер `_classifier_prompt` (фразы из БД попадают в промпт; без триггеров блок-подсказок не выводится).
...
#### Fixed
- **«Сумма частей» при разделении дохода считалась неверно.** Авто-балансируемая часть форматировалась локале-зависимо (`"%.2f".format` → запятая в RU-локали), а `toDoubleOrNull()` парсит только точку — поэтому пересчитанная часть учитывалась как 0, пока её вручную не перетронешь. `formatAmountInput` теперь форматирует через `Locale.US` (точка). Кнопка «Разделить» снова доступна сразу при корректной сумме.

#### Changed
- **Bulk-выделение: контент выбранной карточки приглушается** (`alpha 0.4`, анимированно) — чек-кружок `SelectionOverlay` больше не сливается с аватаром/категорией/суммой. Применено к `SwipeableTransactionCard` (Доходы/Расходы) и обеим карточкам Прогноза (регулярные + желания); анимация выделения сохранена.
